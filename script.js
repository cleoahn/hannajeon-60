...
const CONFIG = {
  ...
  // 작업 현황판은 자동 연동이 아닙니다.
  // 네이버카페 미션 인증을 확인한 운영자가 아래 배열만 수동으로 추가/수정합니다.
  // 실명 대신 닉네임만 입력해 주세요.
  students: [
    { name: "봄날기록", status: "입장 완료", note: "오늘 첫 입장 완료" },
    { name: "단단한초안", status: "크몽 가입 완료", note: "전문가 등록 버튼 찾는 중" },
    { name: "엄마의노트", status: "목차 작성 중", note: "흩어진 경험이 정리되는 중" },
    { name: "퇴근후작가", status: "기획안 작성 중", note: "타겟 고객을 좁혀보는 중" },
    { name: "차분히한칸", status: "제목 작성 중", note: "제목 후보 3개 완성" },
    { name: "오늘도완성", status: "표지 작성 중", note: "Canva 템플릿을 고르는 중" },
    { name: "나의첫전자책", status: "원고 초안 작성 중", note: "완벽보다 채우기" },
  ...
  // 현재 버전에서는 회원가입/로그인/DB/자동 인증 연동을 구현하지 않습니다.
  // 추후 필요하면 시작 안내의 입장코드 영역에 간단한 확인 기능만 연결할 수 있습니다.
  resources: [
...
function renderGuide() {
  ...
  const faqs = [
    ...
    ["막히면 어떻게 질문하면 되나요?", "혼자 오래 붙잡지 말고 막힌 화면을 캡처해서 네이버카페 질문 게시판에 올려주세요. 어디까지 해봤는지도 한 줄로 적어주시면 더 빨리 도와드릴 수 있습니다."],
    ["진도가 늦어져도 괜찮을까요?", "괜찮습니다. 부담 갖지 말고 자기 속도로 진행하세요. 현황판을 보며 오늘 할 수 있는 한 가지부터 다시 시작하면 됩니다."]
  ];
...
function renderBoard() {
  $("#studentTable").innerHTML = CONFIG.students.map((student) => `
    <tr><td>${student.name}</td><td>${student.status}</td><td>${student.note}</td></tr>`).join("");
}
